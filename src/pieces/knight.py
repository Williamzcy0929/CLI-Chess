...
class Knight:
    ...
    def verify_knight(self, end_row: int, end_col: int) -> bool:
        ...
        return (changed_col == 2 and changed_row == 1) or (changed_col == 1 and changed_row == 2)

    def is_move_legal(self, board, end_row: int, end_col: int) -> bool:
        """Check if the move is legal."""
        if not board.verify_source_and_destination(self.row, self.col, end_row, end_col, self.is_black):
            return False
